...
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт
	...
	|	СУММА(ВКМ_ВыполненныеКлиентуРаботыОбороты.СуммаКОплатеПриход / ВКМ_ВыполненныеКлиентуРаботыОбороты.КоличествоЧасовПриход) КАК Цена,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константа.ВКМ_НоменклатураАбонентскаяПлата КАК ВКМ_НоменклатураАбонентскаяПлата
	|		ПО ВКМ_НоменклатураРаботыСпециалиста.Значение = ВКМ_НоменклатураАбонентскаяПлата.Значение
	|ГДЕ
	...
	Если РезультатЗапроса.Пустой() Тогда			
		Сообщить("Проверьте наличие номенклатуры в обоих константах," + Символы.ПС + "и сумму абонентской платы в договоре!");	
		Отказ = Истина;
	КонецЕсли;
