...
Функция СозданиеРеализацийЗаПериод(ДатаНачала, ДатаОкончания) Экспорт
	...
	// Из задания я понял что одна реализация в месяц. Потому учитывая что период может быть больше месяца
	// создадим массив периодов для удобства.  
	...
	///А теперь пройдемся по периодам. 
	Для Каждого ТекПериод Из МассивПериодов Цикл
		
		// Сначала проверим есть ли уже реализации по договорам за период. Вторые не нужны.
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ
