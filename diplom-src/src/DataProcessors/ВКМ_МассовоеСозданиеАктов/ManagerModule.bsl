		...
		Пока Выборка.Следующий() Цикл
			...
			Если НовыйДокументРеализации.Услуги.Количество()>0 Тогда 
				...
				Исключение
					...
					НовыйДокументРеализации.Записать(РежимЗаписиДокумента.Запись);					
					
					Результат = СтрШаблон("Документ реализации" + НовыйДокументРеализации.Ссылка + 
					                      "по контрагенту:" + НовыйДокументРеализации.Контрагент + "только записан!");
					ОбщегоНазначения.СообщитьПользователю(Результат);					
					
					ЗаписьЖурналаРегистрации("Не удалось провести документ реализации.Ошибка", УровеньЖурналаРегистрации.Ошибка,,,
